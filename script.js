...
document.getElementById("getBestMatch").addEventListener("click", async function () {
  ...
  try {
    ...
    this.disabled = true;
    this.innerHTML =
      '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Finding best match...';
...
document.getElementById("analyseButton").addEventListener("click", async function () {
  ...
  try {
    ...
    window.finalRecommendations = finalRecommendations
      .map((r) => `Candidate: ${r.name}\nRecommendation: ${r.recommendation}`)
      .join("\n\n");
